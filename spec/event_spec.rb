...
describe Ethereum do
  let(:desc) { load_abi_json('event') }

  xit 'throws exception if did not find match function in abi json' do
    abi = Ethereum::Contract.new(desc)
    expect { abi.filters }.to raise_error(NoMethodError)
  end

  xit 'returns bytes when call function in abi json' do
    abi = Ethereum::Contract.new(desc)
